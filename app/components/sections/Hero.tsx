      ...
      <div className="container relative z-10 mx-auto px-4 md:px-8">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-8">
          ...
          >
            ...
            <motion.h1 
              className="font-display text-[42px] leading-[1.1] md:text-[56px] lg:text-[72px] tracking-tight"
            >
              <motion.span variants={fadeUp} className="block text-white">
                Websites That
              </motion.span>
              <motion.span variants={fadeUp} className="block text-blue-primary">
                Generate Growth
              </motion.span>
            </motion.h1>

            <motion.p variants={fadeUp} className="text-text-muted text-lg max-w-xl">
              Bespoke, conversion-focused websites built by a specialist with 8+ years of experience. Not templates — websites that serve as a scalable lead engine for your business.
            </motion.p>

            <motion.div variants={fadeUp} className="flex flex-wrap items-center gap-4 pt-2">
              <a href="#contact" className={cn(buttonVariants({ size: "lg" }), "bg-orange-primary hover:bg-orange-light text-white border-0 text-base h-14 px-8 rounded-full")}>
                Start Your Project
                <ArrowRight className="ml-2 h-5 w-5" />
              </a>
              <a href="#work" className={cn(buttonVariants({ variant: "outline", size: "lg" }), "border-border-subtle bg-bg-card/50 hover:bg-bg-card text-white text-base h-14 px-8 rounded-full backdrop-blur-sm")}>
                View Our Work
              </a>
            </motion.div>

            <motion.div variants={fadeUp} className="pt-8 mt-4 border-t border-border-subtle/50">
              <div className="flex flex-wrap items-center gap-x-8 gap-y-4">
                {STATS.map((stat, i) => (
                  <div key={i} className="flex items-center">
                    <div className="flex flex-col">
                      <span className="text-2xl font-bold text-white">{stat.value}</span>
                      <span className="text-xs text-text-faint uppercase tracking-wider font-mono">{stat.label}</span>
                    </div>
                    {i !== STATS.length - 1 && (
            ...
            <div className="relative rounded-xl border border-border-card bg-bg-card shadow-2xl shadow-blue-primary/5 overflow-hidden">
              ...
              <div className="p-6 md:p-8 bg-bg-card aspect-4/3 flex flex-col gap-6">
                <div className="flex justify-between items-end mb-4">
                  <div>
                    <h3 className="text-text-secondary font-medium font-display text-lg mb-1">Growth Metrics</h3>
                    <p className="text-text-muted text-sm">+240% Monthly Traffic</p>
                  </div>
                ...
                <div className="flex-1 flex items-end gap-3 md:gap-4 mt-auto h-full w-full">
                  {[40, 60, 45, 80, 65, 95, 100].map((height, i) => (
                    <motion.div
                      ...
                      className={cn(
                        "flex-1 rounded-t-sm",
                        i === 6 ? "bg-orange-primary" : i >= 4 ? "bg-blue-primary" : "bg-border-card"
                      )}
                    />
                  ))}
                </div>
